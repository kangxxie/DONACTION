...
(function() {
  ...
  function testAuthState() {
    ...
    console.log('Utente:', user ? `✅ Autenticato come ${user.name} (${user.email}, ruolo: ${user.role})` : '❌ Non autenticato');
  ...
  function testInterceptor() {
    ...
    window.fetch = function(...args) {
      ...
      console.log('Headers presenti:', options.headers ? '✅' : '❌');
      
      if (options.headers) {
        const authHeader = options.headers.get('Authorization');
        console.log('Authorization header:', authHeader ? `✅ ${authHeader}` : '❌ Assente');
      }
  ...
  function monitorDonationService() {
    ...
    console.log('ATTENZIONE: Questa funzione deve essere eseguita DOPO che il componente è stato caricato');
    ...
    console.log('Servizio monitorato. Controlla gli aggiornamenti donazioni nella console.');
  }
