...
async function checkAvailableCodes() {
  try {
    ...
    console.log("\nSTATISTICHE:");
    console.log(
      `   - Codici admin totali: ${adminCodes.length} (${availableAdminCodes.length} disponibili, ${usedAdminCodes.length} utilizzati)`
    );
    console.log(
      `   - Codici team totali: ${teamCodes.length} (${availableTeamCodes.length} disponibili, ${usedTeamCodes.length} utilizzati)`
    );
