...
async function testDebugEndpoints() {
  try {
    ...
    try {
      const campaignResponse = await axios.get(`${API_URL}/campaign/${CAMPAIGN_ID}`);
      ...
      const updateResponse = await axios.post(`${API_URL}/update-campaign/${CAMPAIGN_ID}`, updateData);
      console.log('Update response:', updateResponse.data);
      
      // 4. Verify the update
      const verifyResponse = await axios.get(`${API_URL}/campaign/${CAMPAIGN_ID}`);
      console.log('\n==== Verifying update ====');
